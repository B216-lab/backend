...
type Repository interface {
	...
	IsRespondentKeyAllowed(context.Context, string) (bool, error)
}
...
func (s *Service) Submit(ctx context.Context, in SubmissionInput) (int, error) {
	...
	if normalized.RespondentKey != nil {
		isAllowed, validateErr := s.repo.IsRespondentKeyAllowed(ctx, *normalized.RespondentKey)
		if validateErr != nil {
			return 0, fmt.Errorf("validate respondent key: %w", validateErr)
		}
		if !isAllowed {
			return 0, ValidationError{field: "respondentKey", msg: "is invalid"}
		}
	}
...
func (s *Service) ValidateRespondentKey(ctx context.Context, respondentKey string) (bool, error) {
	trimmedKey := strings.TrimSpace(respondentKey)
	if trimmedKey == "" {
		return false, ValidationError{field: "respondentKey", msg: "is required"}
	}

	isAllowed, err := s.repo.IsRespondentKeyAllowed(ctx, trimmedKey)
	if err != nil {
		return false, fmt.Errorf("validate respondent key: %w", err)
	}
...
func (s *Service) normalize(in SubmissionInput) (Submission, error) {
	...
	if respondentKey := strings.TrimSpace(in.RespondentKey); respondentKey != "" {
		sub.RespondentKey = &respondentKey
	}
